...
dropdown = ctk.CTkOptionMenu(
    ...
    variable=num_var,
    width=60,
    command=lambda val: generate_flights(int(val))
)
dropdown.pack(side="right")

# ---------- FLIGHTS CONTAINER ----------
flights_container = ctk.CTkFrame(main_frame, fg_color="transparent")
...
def generate_flights(count):
    ...
    flights = generate_dummy_flights(count)
...
def create_flight_frame(flight_data):
    ...
    flight_label = ctk.CTkLabel(
        frame,
        text=f"Flight No: {flight_data['flight_no']}",
    ...
    route_label = ctk.CTkLabel(
        frame,
        text=f"{flight_data['from']}  →  {flight_data['to']}",
        font=("Segoe UI", 16)
    )
    route_label.place(relx=0.98, rely=0.25, anchor="e")

    # Right bottom (Date & Time)
    datetime_label = ctk.CTkLabel(
        frame,
        text=f"Date: {flight_data['date']}   Time: {flight_data['time']}",
        font=("Segoe UI", 14)
    )
